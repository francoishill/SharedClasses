...
/// All code taken from fastJSON project downloaded
namespace SharedClasses//fastJSON
{
	...
	public class JSON
	{
		...
		private object FastCreateInstance(Type objtype)
		{
			...
			{
				//Might be because the class does not have public empty constructor new()
		...
		private DateTime CreateDateTime(string value)
		{
			if (value.Length != "yyyy-MM-dd HH:mm:ss".Length)
			{
				...
				if (double.TryParse(value, out dbl))
					return ConvertFromUnixTimestamp(dbl).ToLocalTime();
			}
